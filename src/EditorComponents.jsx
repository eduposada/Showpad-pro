...
import { 
  Trash2, ArrowUp, ArrowDown, Plus, X, Download, Share2, 
  CheckCircle2, Globe, Lock, Users 
} from 'lucide-react';
...
export const MainEditor = ({ item, songs, triggerDL, onClose, onShow, refresh, styles }) => {
    // Estados locais para evitar o bug do cursor pulando
    ...
    // Estados para Gestão de Banda
    const [myBands, setMyBands] = useState([]);
    const [selectedBand, setSelectedBand] = useState(item.data.band_id || "");

    // Busca as bandas que o usuário participa para encher o seletor
    useEffect(() => {
        db.my_bands.toArray().then(setMyBands);
    }, []);

    // Função Unificada de Salvamento (Local + Nuvem)
    const save = async () => {
        ...
        // 1. Salva no Banco Local (Dexie) - Garante funcionamento offline
        if (isSong) await db.songs.update(item.data.id, changes);
        else await db.setlists.update(item.data.id, changes);
        
        // 2. Sincroniza com a Nuvem (Supabase) se uma banda estiver selecionada
        if (selectedBand && supabase) {
            ...
            if (error) console.error("Erro na sincronização cloud:", error.message);
        }
        refresh();
    };

    // --- RENDERIZAÇÃO DO EDITOR DE SETLIST (SHOWS) ---
            ...
            <div style={styles.editorContent}>
                <div style={styles.editorHeader}>
                    <div style={{flex:1}}>
                        ...
                        <select 
                            style={{background:'#1a1a1a', color:'#007aff', border:'none', fontSize:'11px', marginTop:'5px', outline:'none', cursor:'pointer'}}
                            value={selectedBand}
                            onChange={(e) => { setSelectedBand(e.target.value); save(); }}
                        >
                            <option value="">🔒 Privado (Só eu vejo)</option>
                            {myBands.map(b => <option key={b.id} value={b.id}>👥 Banda: {b.name}</option>)}
                        </select>
                    </div>
                    <div style={styles.btnGroup}>
                        <button style={styles.exportBtn} onClick={()=>triggerDL({songs: item.data.songs, setlists:[{...item.data}]}, `ShowPad_Show_${lT}.json`)}>EXPORTAR SHOW</button>
                ...
                <div style={styles.showMetaData}>
                    ...
                    <input placeholder="Integrantes da Banda" value={lMem} onChange={e=>setLMem(e.target.value)} onBlur={save} style={styles.metaInputWide}/>
                    <textarea placeholder="Notas e Observações..." value={lNot} onChange={e=>setLNot(e.target.value)} onBlur={save} style={styles.metaTextArea}></textarea>
                </div>

                <div style={styles.setlistSplit}>
                    <div style={styles.setlistHalf}>
                        <h3 style={{color:'#007aff', margin:'0 0 10px 0', fontSize:'14px'}}>Set List do Show</h3>
                        {(item.data.songs || []).map((s, i) => (
                            <div key={i} style={styles.miniItemReorder}>
                                <div style={{flex:1, color:'#fff'}}><b>{i+1}.</b> {s.title}</div>
                                <div style={styles.reorderControls}>
                                    <button onClick={async ()=>{
                                        const n=[...item.data.songs]; 
                                        if(i>0){ [n[i],n[i-1]]=[n[i-1],n[i]]; await db.setlists.update(item.data.id,{songs:n}); refresh(); }
                                    }}><ArrowUp size={14}/></button>
                                    <button onClick={async ()=>{
                                        const n=[...item.data.songs]; 
                                        if(i<n.length-1){ [n[i],n[i+1]]=[n[i+1],n[i]]; await db.setlists.update(item.data.id,{songs:n}); refresh(); }
                                    }}><ArrowDown size={14}/></button>
                                    <button onClick={async ()=>{
                                        const n=[...item.data.songs]; n.splice(i,1); 
                                        await db.setlists.update(item.data.id,{songs:n}); refresh();
                    ...
                    <div style={{...styles.setlistHalf, background:'#222'}}>
                        <h3 style={{color:'#888', margin:'0 0 10px 0', fontSize:'14px'}}>Sua Biblioteca (Clique no +)</h3>
                        {songs.map(s => (
                            <div key={s.id} style={styles.miniItem} onClick={async ()=>{
                                const n=[...(item.data.songs||[]), s]; 
                                await db.setlists.update(item.data.id,{songs:n}); refresh();
                            }}>
    ...
    // --- RENDERIZAÇÃO DO EDITOR DE MÚSICA (SONGS) ---
    return (
        <div style={styles.editorContent}>
            <div style={styles.editorHeader}>
                <div style={{flex:1}}>
                    ...
                    <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
                        ...
                        <select 
                            style={{background:'none', color:'#007aff', border:'none', fontSize:'11px', outline:'none', cursor:'pointer'}}
                            value={selectedBand}
                            onChange={(e) => { setSelectedBand(e.target.value); save(); }}
                        >
                ...
                <div style={styles.btnGroup}>
                    <button style={styles.exportBtn} onClick={() => triggerDL({songs:[{...item.data, content:lC, title:lT, artist:lA}]}, `ShowPad_Musica_${lT}.json`)}>EXPORTAR</button>
