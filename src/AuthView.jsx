...
export const AuthView = ({ styles }) => {
    ...
    const handleLogin = async (e) => {
        ...
        else if (!usePassword) alert('Verifique seu e-mail para o link de acesso!');
        setLoading(false);
    };
    ...
    if (!supabase) return (
        <div style={styles.wizard}>
            <div style={styles.settingsCard}>
                ...
                <p style={{textAlign:'center'}}>Chaves do Supabase não encontradas no .env ou Vercel.</p>
            </div>
        </div>
    );
        ...
        <div style={styles.wizard}>
            <div style={styles.settingsCard}>
                ...
                <form onSubmit={handleLogin} style={{display:'flex', flexDirection:'column', gap:'15px', marginTop:'20px'}}>
                    ...
                    {usePassword && <input style={styles.inputField} type="password" placeholder="Sua senha" value={password} onChange={(e) => setPassword(e.target.value)} required />}
                ...
                <button onClick={() => setUsePassword(!usePassword)} style={{marginTop:'20px', background:'none', border:'none', color:'#007aff', cursor:'pointer', fontSize:'12px'}}>
                    {usePassword ? "Voltar para Login por E-mail" : "Entrar com Senha"}
                </button>
            </div>
        </div>
    );
